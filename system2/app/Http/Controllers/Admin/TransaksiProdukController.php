...
class TransaksiProdukController extends Controller
{
    ...
    public function refundTransaksiProduk(Request $request, $id)
    {
        ...
        try{
            ...
            if($users->roles[0]->id == 3){
                if($users->referred_by != null){
                    $user_ref = User::where('id',$users->referred_by)->first();
                    if($user_ref->roles[0]->id == 4){
                        $saldo_baru = $user_ref->saldo - $transaksi->harga_markup;
                        
                        $user_ref->saldo = $saldo_baru;
                        $user_ref->save();
                        
                        $user_ref->refresh();
                        
                        $mutasi_ref = new Mutasi();
                        $mutasi_ref->user_id = $user_ref->id;
                        $mutasi_ref->trxid   = $transaksi->id;
                        $mutasi_ref->type    = 'debit';
                        $mutasi_ref->nominal = $transaksi->harga_markup;
                        $mutasi_ref->saldo   = $user_ref->saldo;
                        $mutasi_ref->note    = 'REFUND KOMISI TRX REFERRAL USER'.$users->name.' | '.$produk->product_name.' '.$transaksi->target;
                        $mutasi_ref->save();
                    }
                }
            }
                    ...
                    if($users->roles[0]->id == 3){
                        if($users->referred_by != null){
                            ...
                            if($user_ref->roles[0]->id == 4){
                                ...
                                $mutasi_ref = new Mutasi();
                                $mutasi_ref->trx_id  = $transaksi->id;
                                $mutasi_ref->user_id = $user_ref->id;
                                $mutasi_ref->type    = 'debit';
                                $mutasi_ref->nominal = $transaksi->harga_markup;
                                $mutasi_ref->saldo   = $user_ref->saldo;
