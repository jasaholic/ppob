...
class ProfilController extends Controller
{
	...
	public function apiCredentials(Request $request)
	{
	    ...
	    return view('member.profile.api-credentials', compact('user'));
	}
	...
	public function updateApiCredentials(Request $request)
	{
	    ...
	    $data = $request->only('whitelist_ip');
	    
	    $data['whitelist_ip'] = str_replace(' ', '', $data['whitelist_ip']);
	    $data['whitelist_ip'] = array_filter(explode(',', $data['whitelist_ip']), function($ip) {
	        return filter_var($ip, FILTER_VALIDATE_IP) !== false;
	    });
	    $data['whitelist_ip'] = implode(',', $data['whitelist_ip']);
	    
	    if( $request->update_key == '1' ) {
	        $data['api_key'] = Str::random(40);
	    }
	    
	    if( $user->update($data) ) {
	        return redirect()->back()->with('alert-success', 'Berhasil memperbarui data kredensial API');
	    }
	    
	    return redirect()->back()->with('alert-error', 'Gagal memperbarui data kredensial API');
	}
