...
class PembelianController extends Controller
{
    ...
    public function index($title)
    {
        ...
        if ( count($kategori->pembelianoperator) == 1 )
        {
            ...
            return view('member.pembelian.form', compact('kategori', 'operator', 'antrian', 'transaksi','trxForOption','transaksisMobile','komisi'));
        }
        else
        {
            return view('member.pembelian.form', compact('kategori', 'antrian', 'transaksi','trxForOption','transaksisMobile','komisi'));
        }
    }    
    ...
    {
        $data        = $request->q;
        $suggestions = Transaksi::select('mtrpln')->where('user_id', Auth::user()->id)->where('mtrpln', 'LIKE',"%{$data}%")->orderBy('created_at', 'DESC')->limit(10)->get();
    ...
    public function prefixproduct(Request $request)
    {
        ...
        else
        {
            $pembelianoperator = Pembelianoperator::where('pembeliankategori_id', $pembeliankategori_id)
                ->where(function($p) use ($prefix, $prefix4) {
                    $p->where('prefix', 'LIKE', $prefix.',%')
                        ->orWhere('prefix', 'LIKE', '%,'.$prefix.',%')
                        ->orWhere('prefix', 'LIKE', '%,'.$prefix)
                        
                        ->orWhere('prefix', 'LIKE', $prefix4.',%')
                        ->orWhere('prefix', 'LIKE', '%,'.$prefix4.',%')
                        ->orWhere('prefix', 'LIKE', '%,'.$prefix4);
                })
            ...
            $operator = Pembelianoperator::where('pembeliankategori_id', $pembeliankategori_id)
                ->where(function($p) use ($prefix, $prefix4) {
                    $p->where('prefix', 'LIKE', $prefix.',%')
                        ->orWhere('prefix', 'LIKE', '%,'.$prefix.',%')
                        ->orWhere('prefix', 'LIKE', '%,'.$prefix)
    ...
    public function findproduct(Request $request)
    {
     ...
     public function produkLevel($id)
     {
        ...
        {
            $produk = Pembelianproduk::select('id', 'product_id', 'product_name', 'desc', 'price', 'status')->where('pembelianoperator_id', $id)->orderBy('price_default', 'ASC')->get();
        }
    ...
    public function orderproduct(Request $request)
    {
        ...
        $v = Validator::make($request->all(), [
            ...
            //'g-recaptcha-response.required' => 'Token keamanan tidak valid',
            //'g-recaptcha-response.captcha' => 'Token keamanan tidak valid',
        ]);
        ...
        if( $v->fails() ) {
         ...
         $ltx = Antriantrx::where('code', $request->produk)->where('target', $request->target)->where('created_at', '>=', Carbon::now()->subMinutes(5)->toDateTimeString())->count();
            if( $ltx > 0 ) {
