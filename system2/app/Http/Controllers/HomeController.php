...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('home', compact('pembeliankategori','operator', 'testimonials', 'faqs','produkPLN','tagihan_providers', 'captcha'));
    }
    ...
    public function produkLevel($id)
    {
        if($id == 10)
        {
            ...
            {
                $produk = Personal::select('id','product_id','product_name','desc','.price','status')->where('pembeliankategori_id', $id)->orderBy('price_default', 'ASC')->get();
            }
            elseif(Auth::user()->roles()->first()->id == $this->agen_role)
            {
                $produk = Agen::select('id','product_id','product_name','desc','price','status')->where('pembeliankategori_id', $id)->orderBy('price_default', 'ASC')->get();
            }
            elseif(Auth::user()->roles()->first()->id == $this->enterprise_role)
            {
                $produk = Enterprise::select('id', 'product_id', 'product_name', 'desc', 'price', 'status')->where('pembeliankategori_id', $id)->orderBy('price_default', 'ASC')->get();
            }
            else
            {
                $produk = Pembelianproduk::where('pembeliankategori_id', $id)->orderBy('price_default', 'ASC')->get();
        ...
        else
        {
            ...
            elseif(Auth::user()->roles()->first()->id == $this->enterprise_role)
            {
                $produk = Enterprise::select('id', 'product_id', 'desc', 'price', 'status')->where('pembelianoperator_id', $id)->orderBy('price_default', 'ASC')->get();
            }
            else
            {
                $produk = Pembelianproduk::where('pembelianoperator_id', $id)->orderBy('price_default', 'ASC')->get();
    ...
    public function tos()
    {
        $page = staticPage::where('slug','tos')->firstOrFail();
    ...
    {
        $page = staticPage::where('slug','privacy-policy')->firstOrFail();
